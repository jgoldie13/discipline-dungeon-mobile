    ...
    <div className="min-h-screen bg-transparent text-dd-text">
      ...
      <div className="p-6 space-y-6">
        <div className="glass-panel rounded-xl p-6 space-y-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <label className="text-sm text-dd-muted font-medium">
                🌙 Bedtime (last night)
              </label>
              <input
                type="time"
                ...
                className="w-full bg-dd-surface/60 border border-dd-border/60 rounded-lg p-4 text-2xl font-bold text-center text-dd-text focus:outline-none focus:border-mana/50"
                required
              />
            </div>
            ...
            {duration && (
              <div className="scroll-card p-4 text-center">
                <div className="text-sm text-dd-muted">Sleep Duration</div>
                <div className="text-3xl font-bold text-dd-text">
                  {duration} hours
                </div>
            ...
            <div className="space-y-3">
                ...
                {[1, 2, 3, 4, 5].map((rating) => (
                  <button
                    ...
                    className={`flex-1 py-3 rounded-lg font-semibold transition-all border ${
                      restedRating === rating
                        ? 'bg-mana/20 text-dd-text border-mana/40 scale-105'
                        : 'bg-dd-surface/60 text-dd-muted border-dd-border/60 hover:border-mana/30'
                    }`}
                  >
                    {rating}
                  </button>
              ...
              <div className="flex justify-between text-xs text-dd-muted/70">
                <span>😴 Exhausted</span>
                <span>💪 Energized</span>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <div className="text-left">
                    <div className="font-semibold text-dd-text">Energy Equation (Advanced)</div>
                    <div className="text-xs text-dd-muted">Track factors that affect your HP</div>
                  </div>
                </div>
                <span className="text-2xl text-dd-muted">{showAdvanced ? '−' : '+'}</span>
              </button>
              ...
              {showAdvanced && (
                <div className="space-y-4 scroll-card p-4">
                  {/* Alcohol */}
                  <div className="space-y-2">
                    <label className="text-sm text-dd-muted font-medium flex items-center gap-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-2">
                      <label className="flex items-center gap-2 bg-dd-surface/60 border border-dd-border/60 p-3 rounded cursor-pointer">
                        <input
                          type="checkbox"
                          ...
                          className="w-5 h-5"
                        />
                        <span className="text-dd-text">Had caffeine after 2pm <span className="text-xs text-gold">-5 HP</span></span>
                      </label>
                      <div className="space-y-1">
                        <label className="text-xs text-dd-muted/70">Hours before bed (0 = none):</label>
                        <input
                          type="number"
                          min="0"
                          max="24"
                  ...
                  <div className="space-y-2">
                    <label className="flex items-center gap-2 bg-dd-surface/60 border border-dd-border/60 p-3 rounded cursor-pointer">
                      <input
                        ...
                        className="w-5 h-5"
                      />
                      <span className="text-dd-text">☀️ Got outdoor light this morning <span className="text-xs text-mana">+5 HP</span></span>
                    </label>
                  </div>
                  ...
                  <div className="space-y-2">
                    <label className="flex items-center gap-2 bg-dd-surface/60 border border-dd-border/60 p-3 rounded cursor-pointer">
                      <input
                        ...
                        className="w-5 h-5"
                      />
                      <span className="text-dd-text">🏃 Exercised yesterday</span>
                    </label>
                    {exercisedToday && (
                      <div className="space-y-1">
                        <label className="text-xs text-dd-muted/70">Hours before bed (0 = morning):</label>
                        <input
                          type="number"
                          min="0"
                          max="24"
            ...
            {hpPreview !== null && (
              <div className="scroll-card p-4">
                <div className="text-sm text-dd-muted mb-2">Your HP Today:</div>
            ...
            <Link
              href="/mobile"
              className="block w-full text-center text-dd-muted hover:text-dd-text py-2"
            >
              Cancel
            </Link>
          </form>
        ...
        <div className="scroll-card p-4 text-sm">
          <div className="font-semibold mb-2 text-dd-text">💡 Why HP Matters:</div>
