...
struct ContentView: View {
  ...
  private func debugExtensionDiscovery() {
    ...
    if let extensionURL = Bundle.main.url(forResource: "ScreenTimeReportExtension", withExtension: "appex", subdirectory: "PlugIns") {
      ...
      if let infoDict = NSDictionary(contentsOf: infoPlistURL) as? [String: Any] {
        print("Extension Info.plist loaded")
        if let nsExtension = infoDict["NSExtension"] as? [String: Any] {
          print("  NSExtensionPointIdentifier:", nsExtension["NSExtensionPointIdentifier"] ?? "missing")
          print("  NSExtensionPrincipalClass:", nsExtension["NSExtensionPrincipalClass"] ?? "missing")
        } else {
          print("  ✗ NSExtension key missing")
        }
        print("  CFBundleIdentifier:", infoDict["CFBundleIdentifier"] ?? "missing")
        print("  CFBundleExecutable:", infoDict["CFBundleExecutable"] ?? "missing")
      } else {
